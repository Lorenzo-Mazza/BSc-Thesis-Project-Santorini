...
public class AthenaTest {
    ...
    public void nameTest() {
        assertEquals(GodsNames.ATHENA, god.getName());
    }
    ...
    // Testing Athena's power: check that blocked opponents cannot move up in their turn
    @Test
    public void testAthenaMoveUp() {
        owner.selectWorker(owner.getWorkers()[0]);
        Player opponent = new Player("opp");
        owner.addOpponent(opponent);
        owner.getSelectedWorker().setPosition(board.getSpace(0, 0));
        board.getSpace(1,1).setHeight(Block.LEVEL1);
        god.move(owner, board.getSpace(1, 1));
        assertEquals(board.getSpace(1, 1), owner.getSelectedWorker().getPosition());
        assertTrue(opponent.isPlayerBlocked());
    }
